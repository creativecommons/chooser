...
export default {
  ...
  methods: {
    ...
    isLicenseAttribute(stepName) {
      return ['BY', 'NC', 'ND', 'SA'].includes(stepName)
    },
    /**
    ...
    changeStepSelected({ name, id, selected }) {
      ...
      this.$set(this.steps, id, { ...this.steps[id], selected })
    ...
    setStepsVisible(stepsToSetVisible) {
      ...
      this.steps.forEach((step) => {
        if (stepsToSetVisible.includes(step.name) && !step.visible) {
          this.$set(this.steps, step.id, { ...step, visible: true })
        } else if (!stepsToSetVisible.includes(step.name) && step.visible) {
    ...
    setStepsEnabled(stepsToSetEnabled, disabledDue) {
      ...
      const shouldSetEnabled = (step) =>
        stepsToSetEnabled.includes(step.name) && !step.enabled
