...
export default {
  ...
  computed: {
    ...mapGetters(['fullName']),
    tabIndex() {
      return this.step.status === 'completed' ? { tabindex: 0 } : {}
    },
    ...
    completedStepCaption() {
      ...
      const reversed = (name) => ['NC', 'ND', 'SA'].includes(name)
      let captionKey
      if (name === 'DD') {
        return this.fullName
      } else if (['FS', 'CW'].includes(name)) {
        captionKey = selected
          ? `stepper.${name}.selected`
          : `stepper.${name}.not-selected`
...
.step-header__title.completed.disabled,
.step-header__title.inactive {
  color: #b0b0b0;
}
