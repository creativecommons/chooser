...
function shortToFull(shortLicenseName) {
    ...
    if (short.includes('sa')) { base += '-ShareAlike' } else if (short.includes('nd')) { base += '-NoDerivatives' }
...
function fullToShort(fullLicenseName) {
    ...
    if (full.includes('sharealike')) { base += '-SA' } else if (full.includes('noderivatives')) { base += '-ND' }
    // eslint-disable-next-line
    return base += ' 4.0'
}
