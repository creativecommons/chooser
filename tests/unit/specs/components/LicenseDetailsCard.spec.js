...
describe('LicenseDetailsCard.vue', () => {
  ...
  it('Check if the license short name is displayed correctly', () => {
    ...
    expect(shortNameElement.findAll('.icon').length).toEqual(
      currentLicense.ICONS.length,
    );
  });
  ...
  it('Check if the license description is displayed correctly', () => {
    ...
    expect(fullDescription.text()).toEqual(
      'license-details-card.full-description.cc-by-nc-nd',
    );
  });

  it('Check if the license description is displayed correctly', () => {
    const itemsDescription = wrapper.find('.items-description');
    expect(itemsDescription.findAll('li').length).toEqual(
      currentLicense.ICONS.length - 1,
    );
  });
