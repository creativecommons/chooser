...
describe('license url', () => {
  Object.values(LICENSES).forEach((license) => {
    ...
    it(`${JSON.stringify(
      attr
    )} for web should return ${url} with a ref value by default`, () => {
    ...
    it('Should raise an error if BY is not selected', () => {
      expect(() => {
        licenseURL({ ...LICENSES.CC0.ATTRIBUTES, BY: undefined })
      }).toThrowError()
    })
  })
})
      ...
      Object.keys(attributionOptions).forEach((option) => {
        describe(`${option} with should return correct HTML`, () => {
          ...
          const generatedHtml = generateHTML(
            attributionDetails,
            currentLicense.SHORT,
            useFullName,
            isTitleDefault
          )
          const wrapper = mount(TestComponent, {
            propsData: { attribution: generatedHtml },
          })
          const expectedWorkTitle =
            currentAttributionOptions.workTitle || DEFAULT_TITLE
          const expectedCreatorName =
            currentAttributionOptions.creatorName || ''
          const expectedWorkUrl = currentAttributionOptions.workUrl || ''
          const expectedCreatorProfileUrl =
            currentAttributionOptions.creatorProfileUrl || ''
          ...
          it(`has correct license link ${currentLicense.SHORT} and license icons `, () => {
            ...
            expect(licenseLinkElement.attributes('href')).toBe(
              currentLicense.URL + chooserRef
            )
          ...
          it(`has correct work attribution: "${expectedWorkTitle}" at url "${expectedWorkUrl}"`, () => {
            ...
            if (!expectedWorkUrl) {
              ...
              expect(titleLink.attributes().href).toEqual(
                `http://${expectedWorkUrl}`
              )
            }
          })
          it(`has correct creator attribution: "${expectedCreatorName}" at url "${expectedCreatorProfileUrl}"`, () => {
            ...
            if (expectedCreatorName) {
              if (expectedCreatorProfileUrl) {
                ...
                expect(creatorLinkAttributes.rel).toEqual(
                  'cc:attributionURL dct:creator'
                )
                expect(creatorLinkAttributes.property).toEqual(
                  'cc:attributionName'
                )
                expect(creatorLinkAttributes.href).toEqual(
                  `http://${expectedCreatorProfileUrl}`
                )
              } else {
                expect(creator.find('span').attributes().property).toEqual(
                  'cc:attributionName'
                )
              }
            } else {
