...
Cypress.Commands.add('clickNext', () => {
  ...
  cy.get('body')
    .then(($body) => {
      ...
      } else startingCommand = cy;
    })
    .then(() => {
      startingCommand.get('button').contains('NEXT').click();
    });
});
Cypress.Commands.add('clickBack', () => {
  ...
  cy.get('body')
    ...
    .then(() => {
      startingCommand.get('button').contains('BACK').click();
    });
});
...
Cypress.Commands.add('makeAChoice', (stepId, choice) => {
  ...
  cy.get('body')
    ...
    .then(() => {
      startingCommand.get(stepId).find(`[value="${choice}"]`).check();
    });
});
...
Cypress.Commands.add('copyButton', (text) => {
  ...
  cy.get('body')
    ...
    .then(() => {
      startingCommand.get('.donate').contains(text).click();
    });
});
...
Cypress.Commands.add('missingInputField', (id) => {
  ...
  cy.get('body')
    ...
    .then(() => {
      startingCommand
        .get(`:nth-child(${id}) > label.is-normal > .control-inner > .input`)
        .should('not.exist');
    });
});
